...
contract('MasterChef', ([alice, bob, carol, referrer, treasury, dev, fee, owner]) => {
    beforeEach(async () => {
        ...
        this.referral = await CerberusReferral.new({ from: owner });
        this.chef = await MasterChef.new(this.cerberus.address, '100', '1000', { from: owner });

        await this.cerberus.transferOwnership(this.chef.address, { from: owner });
        await this.referral.updateOperator(this.chef.address, true, { from: owner });
        await this.chef.setCerberusReferral(this.referral.address, { from: owner });
