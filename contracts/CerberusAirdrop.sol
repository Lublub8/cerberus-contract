...
contract CerberusAirdrop is Ownable, IMerkleDistributor
{
    ...
    function claim(uint256 index, address account) external override {
        ...
        require(!isClaimed(index), 'CerberusAirdrop: Drop already claimed.');


        // Mark it claimed and send the token.
        _setClaimed(index);
        require(BEP20(token).transfer(account, amount), 'CerberusAirdrop: Transfer failed.');
